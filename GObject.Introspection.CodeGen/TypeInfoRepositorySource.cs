...
namespace GObject.Introspection.CodeGen
{
    ...
    class TypeInfoRepositorySource : ITypeInfoSource
    {
        ...
        readonly Lazy<Dictionary<QualifiedTypeName, Element>> index;
        readonly ConcurrentDictionary<QualifiedTypeName, TypeInfo> cache = new ConcurrentDictionary<QualifiedTypeName, TypeInfo>();
        ...
        public TypeInfoRepositorySource(SyntaxGenerator syntax, IRepositoryProvider repositories)
        {
            ...
            index = new Lazy<Dictionary<QualifiedTypeName, Element>>(() =>
                repositories.GetRepositories()
                    .SelectMany(i => i.Namespaces
                        .SelectMany(j => Enumerable.Empty<Element>()
                            ...
                            .Concat(j.Unions)
                            .OfType<IHasName>()
                            .Select(i => new { Name = new QualifiedTypeName(j.Name, i.Name), Element = (Element)i })))
        ...
        /// <returns></returns>
        public TypeInfo Resolve(QualifiedTypeName name)
        {
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        TypeInfo BuildTypeInfo(QualifiedTypeName name)
        {
            ...
            if (element is Alias alias)
                return Resolve(QualifiedTypeName.Parse(alias.Type.Name, name.Namespace));
