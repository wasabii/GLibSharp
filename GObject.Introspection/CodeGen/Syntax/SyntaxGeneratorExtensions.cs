...
namespace GObject.Introspection.CodeGen.Syntax
{
    ...
    static class SyntaxGeneratorExtensions
    {
        ...
        /// <summary>
        /// Generates the syntax to represent a <see cref="Model.ITypeSymbol"/>.
        /// </summary>
        /// <param name="self"></param>
        /// <param name="symbol"></param>
        /// <returns></returns>
        public static SyntaxNode TypeSymbol(this SyntaxGenerator self, Model.ITypeSymbol symbol)
        {
            if (self is null)
            ...
            {
                var a = symbol.TypeArguments.Cast<Model.ITypeSymbol>();
                var p = symbol.TypeParameters.Cast<Model.ITypeSymbol>();
                return self.GenericName(symbol.Name, a.Zip(p, (i, j) => i ?? j).Select(i => TypeSymbol(self, i)));
            }
            else
                return self.DottedName(symbol.Name);
        }
